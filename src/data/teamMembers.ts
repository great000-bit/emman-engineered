import AdebowaleImg from "@/assets/AdebowaleTaofeek.jpeg";
import DanielImg from "@/assets/DanielOyetayo.jpeg";
import VivianImg from "@/assets/MbaVivian.jpeg";
import ZaydImg from "@/assets/ZaydTahir.png";
import GreatEmmanImg from "@/assets/GreatEmman-wori.jpeg";
import SophiaImg from "@/assets/IyeritufuSophiaAdaobim.jpeg";
import OsuagwuImg from "@/assets/OsuagwuGoldenheart.jpeg";
import DavidImg from "@/assets/DavidEsemuruOkeoghene .jpeg";
import GraceImg from "@/assets/GraceGrant.jpeg";
import CharlesImg from "@/assets/CharlesMacanthony.jpeg";
import JenniferImg from "@/assets/JenniferFrank .jpeg";

export const teamMembers = [
  {
    id: "Great Emman-wori",
    name: "Great Emman-wori",
    role: "Founder & Vision Architect",
    bio: "Building Creative Emman Limited at the intersection of creativity, technology, and opportunity. Passionate about crafting impactful digital experiences, nurturing emerging talent, and developing innovative solutions that empower businesses, inspire collaboration, and shape the future of digital innovation.",
    stack: ["Next.js", "Figma", "Meta Business Suite", "Adobe Creative Cloud", "Sony Cinema Line", "DaVinci Resolve", "After Effects", "Lightroom"],
    image: GreatEmmanImg,
    linkedin: "https://www.linkedin.com/in/great-emman-wori",
    portfolio: "https://creative-emman.vercel.app",
    email: "greatemmanwori@gmail.com",
  },
  {
    id: "Zayd Tahir",
    name: "Zayd Tahir",
    role: "Senior Frontend Developer",
    bio: "Senior Frontend Developer with expertise in React, Next.js, TypeScript, and modern web technologies. Passionate about crafting fast, responsive, and visually appealing user experiences while building scalable and maintainable applications.",
    stack: ["ReactJs", "NextJs", "TypeScript", "JavaScript", "Version Control", "Shadcn/UI", "Framer Motion", "TailwindCSS"],
    image: ZaydImg,
    linkedin: "https://www.linkedin.com/in/zayd-tahir-1376073a8",
    portfolio: "https://zayd-portfolio.vercel.app/",
  },
  {
    id: "Adebowale Taofeek",
    name: "Adebowale Taofeek",
    role: "Graphics Designer",
    bio: "Crafting your ideal visual solution",
    stack: ["Coreldraw", "Photoshop", "Illustrator"],
    image: AdebowaleImg,
    linkedin: "https://www.linkedin.com/in/taofeek-adebowale-725151381",
    portfolio: "https://drive.google.com/drive/folders/1fP7uOoahh9dv_bo8W8ITqtYdMWCrdejK",
  },
  {
    id: "Daniel Oyetayo",
    name: "Daniel Oyetayo",
    role: "Graphic Designer",
    bio: "Helping brands stand out through thoughtful design.",
    stack: ["Photoshop", "Illustrator", "Lightroom", "Figma", "Canva"],
    image: DanielImg,
    linkedin: "https://shorturl.at/MpaS3",
    portfolio: "https://shorturl.at/961Hh",
  },
  {
    id: "MBA Vivian",
    name: "MBA Vivian",
    role: "Virtual Assistant & Content Writer",
    bio: "Turning Ideas into Action, One Detail at a Time.",
    stack: ["Productivity Tool", "Communication Tool", "Project Management", "Design"],
    image: VivianImg,
    linkedin: "https://bit.ly/4a6BtDp",
    portfolio: "https://shorturl.at/961Hh",
  },
  {
    id: "Osuagwu Goldenheart",
    name: "Osuagwu Goldenheart",
    role: "Mobile Photographer",
    bio: "Reimagining life through the lens.",
    stack: ["Notion", "Snapseed", "Claude"],
    image: OsuagwuImg,
    linkedin: "",
    portfolio: "",
  },
  {
    id: "David Esemuru Okeoghene",
    name: "David Esemuru Okeoghene",
    role: "Graphic Designer & Social Media Manager",
    bio: "Helping businesses and startups grow their social presence, capture leads, and make an impact through thoughtful, high-converting design",
    stack: ["Canva", "Desygner", "Adobe Express", "Capcut", "Business Suite", "Facebook Studio", "Chatgpt", "Gemini Ai"],
    image: DavidImg,
    linkedin: "",
    portfolio: "https://drive.google.com/drive/folders/1LBB4ExWBtPB6ZhWZ3RiV80aOmoq22du7",
  },
  {
    id: "Grace Grant",
    name: "Grace Grant",
    role: "Content Creator, Virtual Assistant, Video Editor & Social Media Manager",
    bio: "A Content Creator who tells stories with words and images.",
    stack: ["Canva", "Capcut", "Google Workspace", "Trello", "Asana"],
    image: GraceImg,
    linkedin: "https://www.linkedin.com/in/grace-grant-4b647224b",
    portfolio: "",
  },
  {
    id: "Charles Macanthony",
    name: "Charles Macanthony",
    role: "Frontend Engineer",
    bio: "Aspiring software developer building practical web applications, learning Python, and solving real-world community problems",
    stack: ["ReactJs", "HTML", "CSS", "TailwindCSS", "Firebase", "Supabase"],
    image: CharlesImg,
    linkedin: "https://www.linkedin.com/in/macanthony-charles-561810367",
    portfolio: "https://cmc-tech.netlify.app/",
  },
  {
    id: "Jennifer Frank",
    name: "Jennifer Frank",
    role: "Community Manager",
    bio: "Building connections, one conversation at a time.",
    stack: ["Canva", "ChatGpt", "Google Workspace", "Content Strategy", "Community Engagement"],
    image: JenniferImg,
    linkedin: "https://www.linkedin.com/in/jennifer-frank-00b589372",
    portfolio: "",
  },
];
